// var name1=prompt("player 1 name you will be blue");
// var name1color='rgb(86,151,255)';
// var name2=prompt("player 2 name you will be red");
// var name1color='rgb(237,45,73)';
// var game_on=true;
// var buttons=$('table tr');
// function reportWin(rowNum,colNum){
//   consloe.log(You won starting at this row and column);
//   console.log(rowNum);
//   console.log(colNum);
// }
...
function gameEnd(winningPlayer) {
  for (var col = 0; col < 7; col++) {
    for (var row = 0; row < 7; row++) {
      table.eq(row).find('td').eq(col).find('button').attr("disabled", true);
...
count++;
  ...
  var bottomAvail = checkBottom(col);
  if(bottomAvail===undefined){
  alert("column is full ,now another player,s turn!! its your fault");
count--;
}
  // Drop the chip in that column at the bottomAvail Row
  changeColor(bottomAvail,col,currentColor);
  ...
  if(count===42){
  alert("Game Draw,all columns are full,refresh to pla another game")
  for (var col = 0; col < 7; col++) {
    for (var row = 0; row < 7; row++) {
      table.eq(row).find('td').eq(col).find('button').attr("disabled", true);
